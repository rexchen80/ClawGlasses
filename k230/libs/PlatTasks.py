...
class ClassificationApp(AIBase):
    def __init__(
        ...
        if mode not in ["video", "image"]:
            print(
                "Please select the correct inference mode, including 'video', 'image'."
            )
        ...
        with ScopedTiming("draw result", self.debug_mode > 0):
            ...
            if res["label"] != "":
                draw_img.draw_string_advanced(
                    ...
                    5,
                    32,
                    res["label"] + " " + str(round(res["score"], 3)),
                    color=(0, 255, 0),
                )
    ...
    def config_preprocess(self, input_image_size=None):
        with ScopedTiming("set preprocess config", self.debug_mode > 0):
            ...
            top, bottom, left, right, _ = center_pad_param(
                ai2d_input_size, self.model_input_size
            )
            # 配置padding预处理
        ...
        with ScopedTiming("draw osd", self.debug_mode > 0):
            ...
            if res["boxes"]:
                for i in range(len(res["boxes"])):
                    ...
                    w = int(
                        float(res["boxes"][i][2] - res["boxes"][i][0])
                        * self.display_size[0]
                        // self.rgb888p_size[0]
                    )
                    h = int(
                        float(res["boxes"][i][3] - res["boxes"][i][1])
                        * self.display_size[1]
                        // self.rgb888p_size[1]
                    )
                    draw_img.draw_rectangle(
                        x, y, w, h, color=self.color_four[res["idx"][i]]
                    )
                    draw_img.draw_string_advanced(
                        x,
                        y - 50,
                        24,
                        self.labels[res["idx"][i]]
                        + " "
                        + str(round(res["scores"][i], 2)),
                        color=self.color_four[res["idx"][i]],
                    )
...
class SegmentationApp(AIBase):
    ...
    def postprocess(self, input_np):
        with ScopedTiming("postprocess", self.debug_mode > 0):
            ...
            # 在mask数据上创建osd图像并返回
            if self.mode == "video":
                self.cur_result["mask"] = image.Image(
                    self.display_size[0],
                    self.display_size[1],
                    image.ARGB8888,
...
class OCRDetectionApp(AIBase):
    ...
    def draw_result(self, osd_img, res):
        ...
        for i in range(len(self.cur_result["boxes"])):
            for j in range(4):
                x1 = (
                    self.cur_result["boxes"][i][2 * j]
                    * self.display_size[0]
                    // self.rgb888p_size[0]
                )
                y1 = (
                    self.cur_result["boxes"][i][2 * j + 1]
                    * self.display_size[1]
                    // self.rgb888p_size[1]
                )
                x2 = (
                    self.cur_result["boxes"][i][(2 * j + 2) % 8]
                    * self.display_size[0]
                    // self.rgb888p_size[0]
                )
                y2 = (
                    self.cur_result["boxes"][i][(2 * j + 3) % 8]
                    * self.display_size[1]
                    // self.rgb888p_size[1]
                )
                osd_img.draw_line(
    ...
    def postprocess(self, results):
        with ScopedTiming("postprocess", self.debug_mode > 0):
            ...
            for i in range(len(preds)):
                if preds[i] != (len(self.ocr_dict) - 1) and (
                    not (i > 0 and preds[i - 1] == preds[i])
                ):
                    self.cur_result["text"] = (
                        self.cur_result["text"] + self.ocr_dict[preds[i]]
                    )
            return self.cur_result
    ...
    def draw_result(self, osd_img, det_boxes, ocr_texts):
        ...
        for i in range(len(det_boxes)):
            ...
            osd_img.draw_string_advanced(
                int(x1), int(y1), 24, ocr_texts[i], color=(0, 0, 255)
            )
        ...
        with ScopedTiming("draw osd", self.debug_mode > 0):
            ...
            for i in range(len(res["labels"])):
                osd_img.draw_string_advanced(
                    ...
                    i * 30,
                    24,
                    res["labels"][i] + " " + str(round(res["scores"][i], 3)),
                    color=(255, 0, 255, 0),
                )
