        ...
        with ScopedTiming("init PipeLine", self.debug_mode > 0):
            ...
            if sensor_id is not None:
                self.sensor = (
                    ...
                    else Sensor(id=sensor_id, fps=board_fps)
                )
            else:
            ...
            if self.display_size:
                ...
                Display.init(display_type, osd_num=self.osd_layer_num, to_ide=to_ide)
            ...
            if crop_vertical:
                ...
                # 通道0直接给到显示VO，格式为YUV420
                self.sensor.set_framesize(
                    w=self.display_size[0], h=self.display_size[1], chn=CAM_CHN_ID_0
                )
                self.sensor.set_pixformat(Sensor.YUV420SP, chn=CAM_CHN_ID_0)
                # 通道2给到AI做算法处理，格式为RGB888
                self.sensor.set_framesize(
