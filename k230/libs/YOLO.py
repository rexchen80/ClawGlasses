...
class YOLOv5(AIBase):
    def __init__(
        ...
        if task_type not in ["classify", "detect", "segment"]:
            raise Exception(
                "Please select the correct task_type parameter, including 'classify', 'detect', 'segment'."
            )
        ...
        if self.task_type == "segment":
            if self.mode == "image":
                self.masks = np.zeros(
                    (1, self.rgb888p_size[1], self.rgb888p_size[0], 4), dtype=np.uint8
                )
            elif self.mode == "video":
                self.masks = np.zeros(
                    (1, self.display_size[1], self.display_size[0], 4), dtype=np.uint8
                )
        # Ai2d实例，用于实现模型预处理
    ...
    def config_preprocess(self, input_image_size=None):
        with ScopedTiming("set preprocess config", self.debug_mode > 0):
            ...
            if self.task_type == "classify":
                ...
                # 配置padding预处理
                self.ai2d.pad(
                    [0, 0, 0, 0, top, bottom, left, right], 0, [128, 128, 128]
                )
            elif self.task_type == "segment":
                top, bottom, left, right, scale = letterbox_pad_param(
                    self.rgb888p_size, self.model_input_size
                )
                self.ai2d.pad(
                    [0, 0, 0, 0, top, bottom, left, right], 0, [128, 128, 128]
    ...
    def postprocess(self, results):
        with ScopedTiming("postprocess", self.debug_mode > 0):
            if self.task_type == "classify":
                ...
                if self.mode == "image":
                    det_res = aidemo.yolov5_det_postprocess(
                        results[0][0],
                ...
                if self.mode == "image":
                    seg_res = aidemo.yolov5_seg_postprocess(
                        results[0][0],
                        results[1][0],
                        [self.rgb888p_size[1], self.rgb888p_size[0]],
                        [self.model_input_size[1], self.model_input_size[0]],
                ...
                if self.task_type == "classify":
                    ...
                    if res:
                        ...
                        for i in range(len(res[0])):
                            ...
                            img.draw_rectangle(
                                x, y, w, h, color=self.colors[res[1][i]], thickness=4
                            )
                    ...
                    if res[0]:
                        ...
                        for i, det in enumerate(dets):
                            ...
                            img.draw_string_advanced(
                                ...
                                + " {0:.3f}".format(scores[i]),
                                color=self.colors[int(ids[i])],
                            )
                    else:
                        img.clear()
                else:
...
class YOLOv8(AIBase):
    def __init__(
        ...
        if task_type not in ["classify", "detect", "segment", "obb"]:
            raise Exception(
                "Please select the correct task_type parameter, including 'classify', 'detect', 'segment','obb'."
            )
        super().__init__(kmodel_path, model_input_size, rgb888p_size, debug_mode)
        self.task_type = task_type
        self.mode = mode
    ...
    def postprocess(self, results):
        with ScopedTiming("postprocess", self.debug_mode > 0):
            if self.task_type == "classify":
                ...
                new_result = results[0][0].transpose()
                if self.mode == "image":
                    seg_res = aidemo.yolov8_seg_postprocess(
                        new_result.copy(),
                        results[1][0],
                        [self.rgb888p_size[1], self.rgb888p_size[0]],
                        [self.model_input_size[1], self.model_input_size[0]],
                    ...
                    if res:
                        ...
                        for i in range(len(res[0])):
                            x1, y1, x2, y2, x3, y3, x4, y4 = map(
                                lambda x: int(round(x, 0)), res[0][i]
                            )
                            img.draw_line(
                                int(x1),
                                int(y1),
                            ...
                            img.draw_string_advanced(
                                x1, y1, 24, str(res[1][i]), color=self.colors[res[1][i]]
                            )
                    else:
                        img.clear()
                else:
